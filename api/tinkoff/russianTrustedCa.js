// Russian Trusted CA (Минцифры РФ). Tinkoff Invest API
// (invest-public-api.tinkoff.ru) отдаёт TLS-цепочку, подписанную этим корнем,
// которого НЕТ в trust store Node.js → без него любой запрос падает с
// SELF_SIGNED_CERT_IN_CHAIN (на Vercel и на любой не-РФ инфраструктуре).
// Содержит промежуточный (Russian Trusted Sub CA) и корневой (Russian Trusted
// Root CA) сертификаты. Вшито СТРОКОЙ (не читается из файла), чтобы гарантированно
// попасть в бандл @vercel/node. Извлечено из живой цепочки; совпадает с
// официальными сертификатами Минцифры (gu-st.ru).
export const RUSSIAN_TRUSTED_CA = `
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
`;
